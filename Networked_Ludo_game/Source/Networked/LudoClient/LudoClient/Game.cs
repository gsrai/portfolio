    ...
    public class Game
    {
        ...
        public bool placePiece(int position, Token t)
        {
            ...
            else if (isEnemyPosition(position, t.color))
            {
                ...
                if (this.board[position].color == Player.PlayerColor.Red)
                    this.playerList[0].tokens[this.board[position].id].isInYard = true;

                if (this.board[position].color == Player.PlayerColor.Yellow)
                    this.playerList[1].tokens[this.board[position].id].isInYard = true;

                if (this.board[position].color == Player.PlayerColor.Blue)
                    this.playerList[2].tokens[this.board[position].id].isInYard = true;

                if (this.board[position].color == Player.PlayerColor.Green)
                    this.playerList[3].tokens[this.board[position].id].isInYard = true;
        ...
        public void movePiece(int prevPos, int newPos)
        {
            ...
            if (this.board[newPos] != null && isEnemyPosition(newPos, this.board[prevPos].color))
            {
                ...
                // get the color, and the id of the token
                // then set the correct playerlist token to in yard
                if (this.board[newPos].color == Player.PlayerColor.Red)
                    this.playerList[0].tokens[this.board[newPos].id].isInYard = true;

                if (this.board[newPos].color == Player.PlayerColor.Yellow)
                    this.playerList[1].tokens[this.board[newPos].id].isInYard = true;

                if (this.board[newPos].color == Player.PlayerColor.Blue)
                    this.playerList[2].tokens[this.board[newPos].id].isInYard = true;

                if (this.board[newPos].color == Player.PlayerColor.Green)
                    this.playerList[3].tokens[this.board[newPos].id].isInYard = true;
        ...
        // this resolves the "no ones home home" bug
        public void moveTokenOnPlayerList(Player.PlayerColor color, int id, int homePos)
        {
            if (color == Player.PlayerColor.Red)
                this.playerList[0].tokens[id].position = homePos;

            if (color == Player.PlayerColor.Yellow)
                this.playerList[1].tokens[id].position = homePos;

            if (color == Player.PlayerColor.Blue)
                this.playerList[2].tokens[id].position = homePos;

            if (color == Player.PlayerColor.Green)
                this.playerList[3].tokens[id].position = homePos;
        }
        ...
        public void moveToHomeColumn(int position, int dieResult, Player.PlayerColor color, Token t)
        {
            if (color == Player.PlayerColor.Red)
            {
                ...
                moveTokenOnPlayerList(t.color, t.id, homePos);
