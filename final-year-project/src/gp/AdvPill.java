...
/*
 * Class AdvPill
 * Advances the agent to the nearest uneaten pill. It does this by
 * finding the position of the nearest uneaten pill and then calculating
 * the shortest path to that position, thereby mutating the new direction
 * of the agent.
 * 
 * Evaluate will return the agents facing direction encoded as a modulo 4
 * number.
 */
public class AdvPill extends Sexpr {
	...
	public int evaluate() {
		if (Sexpr.game.player.pillEatenCount < 4) {
			...
			Utils.AdvanceTile(coord[0], coord[1]);
		} 
